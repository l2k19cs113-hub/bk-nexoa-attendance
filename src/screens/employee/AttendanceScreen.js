...
import {
  ...
  Animated, Alert, ActivityIndicator, Dimensions, Platform
} from 'react-native';
...
export default function AttendanceScreen() {
  ...
  useEffect(() => {
    if (profile?.id) fetchTodayAttendance(profile.id);
    ...
    const getLoc = async () => {
      try {
        ...
        // Timeout after 5 seconds for web stability
        ...
        setLocationText('Web Check-in (GPS Optional)');
      }
    };
    getLoc();
  }, [profile?.id]);
  ...
  const performCheckIn = async () => {
    try {
      ...
      Alert.alert('Attendance Error', err.message || 'Check-in failed. Please try again.');
    }
  };
  ...
  const handleAction = () => {
    ...
    // On web, skip the confirmation alert to avoid browser blocks
    if (Platform.OS === 'web') {
      if (isCheckedIn) performCheckOut();
      else performCheckIn();
    } else {
      Alert.alert(`Confirm ${actionType}`, `Do you want to ${actionType.toLowerCase()} now?`, [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Confirm', onPress: isCheckedIn ? performCheckOut : performCheckIn },
      ]);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.body}>
        ...
        <Animated.View style={[styles.btnContainer, { transform: [{ scale: pulseAnim }] }]}>
          ...
          >
            <LinearGradient colors={btn.gradient} style={styles.mainBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              {isLoading ? (
...
const styles = StyleSheet.create({
  ...
  clockTime: { fontSize: 54, fontWeight: '800', color: '#fff' },
  ...
  timesRow: { flexDirection: 'row', width: '100%', backgroundColor: COLORS.bgCard, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.border, marginBottom: 14 },
  ...
  durationBox: { flex: 1, alignItems: 'center', justifyContent: 'center', borderLeftWidth: 1, borderRightWidth: 1, borderColor: COLORS.border, padding: 14 },
  ...
  locationCard: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: COLORS.bgCard, width: '100%', borderRadius: RADIUS.md, padding: 12, borderWidth: 1, borderColor: COLORS.border, marginBottom: 32 },
  locationText: { fontSize: 13, color: COLORS.textMuted, flex: 1 },
  btnContainer: { alignItems: 'center' },
  touchableArea: { borderRadius: 90 },
  mainBtn: {
    width: 180, height: 180, borderRadius: 90, justifyContent: 'center', alignItems: 'center',
    shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.5, shadowRadius: 24, elevation: 20, gap: 8
  },
  mainBtnIcon: { width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center' },
  mainBtnText: { fontSize: 16, fontWeight: '800', color: '#fff', letterSpacing: 0.5 },
  completeBanner: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 24, backgroundColor: `${COLORS.success}15`, borderRadius: RADIUS.md, padding: 14, borderWidth: 1, borderColor: `${COLORS.success}40` },
